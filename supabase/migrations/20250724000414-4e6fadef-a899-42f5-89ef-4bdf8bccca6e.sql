...
-- 1. Remove the dangerous "Anyone can insert specs" policy
DROP POLICY IF EXISTS "Anyone can insert specs" ON public.specs;

-- 2. Create secure RLS policies for specs table that require authentication
...
-- 3. Restore foreign key constraint for specs.uploader_id
...
-- 5. Create audit trigger function for specs table
...
-- 6. Update storage policies to require authentication
...
-- 7. Create a function to safely insert admin profiles
...
DECLARE
  new_user_id UUID;
BEGIN
  -- This function should only be called by existing admins or during setup
  -- Insert a new user into auth.users (this would typically be done through Supabase Auth)
  -- For now, we'll create a placeholder that can be updated when the admin signs up
  ...
  VALUES (new_user_id, admin_email, admin_name, 'admin'::app_role);
